...
final filterNotifierProvider = StateNotifierProvider<FilterNotifier, FilterModel>((ref) {
  final supabase = ref.watch(supabaseServiceProvider);
  return FilterNotifier(supabase);
});

/// Legacy Filter notifier for managing user filters with database persistence
class FilterNotifier extends StateNotifier<FilterModel> {
  final SupabaseService _supabase;
  bool _isLoading = false;
  bool _hasLoaded = false;

  FilterNotifier(this._supabase) : super(FilterModel.defaultFilters) {
    loadFilters();
  }
  ...
  Future<void> loadFilters() async {
    ...
    try {
      final data = await _supabase.getFilters();
      if (data != null && data['filters'] != null) {
        final filtersJson = data['filters'] as Map<String, dynamic>;
        state = FilterModel.fromJson(filtersJson);
      }
      _hasLoaded = true;
    } catch (e) {
      print('Error loading filters: $e');
  ...
  Future<void> _saveFilters() async {
    try {
      await _supabase.saveFilters(state.toJson());
    } catch (e) {
      print('Error saving filters: $e');
    }
  }
  ...
  /// Reset to default filters
  Future<void> resetFilters() async {
    state = FilterModel.defaultFilters;
    await _saveFilters();
  }
